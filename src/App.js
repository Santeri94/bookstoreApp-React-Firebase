...
function App() {
  ...
  const fetchItems = () => {
    fetch('https://bookstore-97ca8-default-rtdb.europe-west1.firebasedatabase.app/books/.json')
  ...
  const deleteBook = (id) => {
    fetch(`https://bookstore-97ca8-default-rtdb.europe-west1.firebasedatabase.app/books/${id}.json`,
    {
      method: 'DELETE',
    })
    .then(response => fetchItems())
      ...
      <div className="ag-theme-material" style={ { height: 400, width: 600, margin: 'auto' } }>
        <AgGridReact rowData={books}>
          ...
          <AgGridColumn 
            ...
            field='id' 
            width={90}
            cellRenderer={ params => // needed to render button inside AgGrid column
